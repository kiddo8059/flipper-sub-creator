#include "sub_creator_gen_info.h"
#include "../helpers/sub_creator_txrx_create_protocol_key.h"
#include <lib/subghz/blocks/math.h>

void sub_creator_gen_info_reset(GenInfo* gen_info) {
    furi_assert(gen_info);
    memset(gen_info, 0, sizeof(GenInfo));
}

void sub_creator_scene_set_type_fill_generation_infos(GenInfo* infos_dest, SetType type) {
    GenInfo gen_info = {0};
    uint64_t key = (uint64_t)rand();

    uint64_t gangqi_key;
    sub_creator_txrx_gen_serial_gangqi(&gangqi_key);

    uint64_t marantec_key;
    sub_creator_txrx_gen_key_marantec(&marantec_key);
